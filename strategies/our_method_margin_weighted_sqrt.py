"""
Our Method Margin Weighted Sqrt - margin 采样 + 下一轮复制式加权训练策略（权重开根号）

该策略保持 `our_method_margin` 的采样流程不变，只修改“下一轮如何训练”：
1. 当前轮仍使用普通训练模型进行覆盖率估计、半径计算与 margin 打分采样；
2. 当本轮采样完成后，基于“当前已标注点的生效半径 + 新选点的生效预测半径”
   在全集上重新做一次覆盖归属；
3. 先按照 `our_method_margin_weighted` 的原始定义，为每个已标注点计算整数复制权重；
4. 再把每个原始权重 `w` 压缩为 `ceil(sqrt(w))`，以缓和过于极端的权重分布；
5. 下一轮训练开始时，不做 loss 加权，而是把已标注样本按该压缩后的整数权重
   重复复制，打乱后按普通 DataLoader 训练。
"""
...
class OurMethodMarginWeightedSqrtStrategy(OurMethodMarginStrategy):
    """
    `our_method_margin` 的复制式加权训练版本（复制权重先开根号再取上整）。

    该类的核心职责不是改变“当前轮如何选点”，而是在每轮选点结束后，为下一轮
    训练缓存一份“复制式训练配置”：
    - 采样阶段完全复用 `OurMethodMarginStrategy` 的行为；
    - 训练阶段先按原始覆盖归属规则得到复制权重；
    - 随后把每个权重 `w` 压缩为 `ceil(sqrt(w))`；
    - 最终把压缩后的整数权重解释为样本重复次数，而不是 loss 系数。

    因此它适合用于你当前的实验目标：
    采样逻辑保持可比，训练机制仍然体现覆盖贡献，但不会让大权重点在复制后
    过度主导下一轮训练。
    """
    ...
    def __init__(
        ...
        """
        初始化 margin + 开根号复制式加权训练策略。

        参数保持与 `OurMethodMarginStrategy` 一致，目的是让实验入口、配置文件
        和现有脚本都可以直接复用，不需要额外为 sqrt 版本拆一套配置字段。
        ...
        self.name = "OurMethodMarginWeightedSqrt"
    ...
    def export_continuation_state(self) -> Dict[str, Any]:
        ...
        除了父类统一导出的 `round_counter` 之外，本策略
        还必须保存与当前已标注索引对齐的 `repeat_counts`。
        这份状态不包含模型参数；`state_dict` 在导出时明确
        固定为 None，保证续跑仍然按“重新初始化并训练模型”
        的当前主流程执行。

        返回:
            Dict[str, Any]: 可序列化的复制式策略续跑状态。
        """
        state = super().export_continuation_state()
        state["weighted_start_round"] = int(self.weighted_start_round)
        if self._next_round_training_setup is None:
            state["next_round_training_setup"] = None
        else:
            state["next_round_training_setup"] = {
                "mode": "replicated",
                "repeat_counts": [
                    int(count)
                    for count in self._next_round_training_setup.get(
                        "repeat_counts", []
                    )
                ],
                "expected_labeled_indices": [
                    int(index)
                    for index in self._next_round_training_setup.get(
                        "expected_labeled_indices", []
                    )
                ],
                "state_dict": None,
    ...
    def restore_continuation_state(self, state: Dict[str, Any]) -> None:
        ...
        恢复时会对 `weighted_start_round` 进行一致性检查，
        并通过现有的 `_cache_next_round_training_setup()` 重建缓存，
        以复用其长度校验与整数转换逻辑。这样 SALTExact
        作为本类子类时也能共用同一恢复语义。

        参数:
            state: `export_continuation_state()` 导出的状态字典。

        返回:
            None
        """
        super().restore_continuation_state(state)
        saved_start_round = int(
            state.get("weighted_start_round", self.weighted_start_round)
        )
        if saved_start_round != self.weighted_start_round:
            raise ValueError(
                "续跑状态的 weighted_start_round 与当前配置不一致: "
                f"{saved_start_round} vs {self.weighted_start_round}"
            )

        training_setup = state.get("next_round_training_setup")
        if training_setup is None:
            self._next_round_training_setup = None
            return
        if training_setup.get("mode") != "replicated":
            raise ValueError("不支持的续跑训练模式")
        raw_repeat_counts = list(training_setup.get("repeat_counts", []))
        repeat_counts = [int(count) for count in raw_repeat_counts]
        if any(
            float(raw_count) != int_count or int_count <= 0
            for raw_count, int_count in zip(raw_repeat_counts, repeat_counts)
        ):
            raise ValueError("续跑状态的 repeat_counts 必须全部为正整数")
        self._cache_next_round_training_setup(
            next_labeled_indices=training_setup.get(
                "expected_labeled_indices", []
            ),
    ...
    def _compute_repeat_counts_for_next_round(
        ...
        """
        计算下一轮训练所需的复制次数列表，并对原始权重做 `ceil(sqrt(w))` 压缩。

        计算规则分两步：
        1. 先完全按照 `our_method_margin_weighted` 的原始语义，基于全集覆盖归属
           计算每个已标注点的原始整数复制权重；
        2. 再把每个原始权重 `w` 变换为 `ceil(sqrt(w))`，以减弱长尾样本在复制
           式训练中的放大效应，同时保留“覆盖更多区域的点应获得更大权重”的顺序。
        ...
        返回:
            List[int]: 与“下一轮已标注索引顺序”严格对齐的压缩后整数复制次数。
        ...
        raw_repeat_counts = get_weight_from_indices_and_radii(
        ...
        sqrt_repeat_counts = np.ceil(
            np.sqrt(np.asarray(raw_repeat_counts, dtype=np.float64))
        )
    ...
    def _prepare_next_round_training_setup(
        ...
        """
        根据当前轮采样结果，为下一轮训练生成并缓存开根号压缩后的复制式训练配置。

        这是本策略区别于 `our_method_margin` 的核心步骤。它把“本轮采样结果”
        转换成“下一轮该如何训练”的显式配置，具体包括：
        1. 组合下一轮的完整已标注集合；
        2. 依据已确认的覆盖归属规则计算每个已标注点的原始复制次数；
        3. 对每个原始权重应用 `ceil(sqrt(w))` 压缩；
        4. 打印当前最大压缩后复制权重，便于调试与实验日志分析；
        5. 把结果缓存到策略内部，供下一轮训练开始时读取。
        ...
        if not self._should_prepare_next_round_training_setup():
            self._next_round_training_setup = None
            print(
                "[OurMethodMarginWeightedSqrt] 跳过下一轮复制权重计算: "
        ...
        if repeat_counts:
            ...
            print(
                "[OurMethodMarginWeightedSqrt] 下一轮最大复制权重: "
                f"{max_weight_value} (idx: {max_weight_index})"
            )
        else:
            print("[OurMethodMarginWeightedSqrt] 下一轮复制权重为空。")
    ...
    def _first_round_selection(
        ...
        """
        执行第一轮采样，并额外为下一轮缓存开根号压缩后的复制式训练权重。
        ...
        - 覆盖归属得到的原始权重会进一步做 `ceil(sqrt(w))` 压缩。

        参数与返回值含义与父类同名方法保持一致。
        """
        if self._should_use_cuda_sampling(device):
            print(f"\n[OurMethodMarginWeightedSqrt] 第一轮采样")
            (
                selected_indices,
                ...
                spectral_norm_product=spectral_norm_product,
                log_prefix="OurMethodMarginWeightedSqrt"
            )
        ...
        if round_indices_array.size > 0:
            ...
            print(
                "[OurMethodMarginWeightedSqrt] 最大理论半径统计: "
        ...
        if labeled_indices:
            ...
            print(f"[OurMethodMarginWeightedSqrt] 原始无标注池大小: {len(unlabeled_indices)}")
            print(
                "[OurMethodMarginWeightedSqrt] 被已标注点覆盖的点数: "
                f"{len(unlabeled_indices) - len(filtered_unlabeled)}"
            )
            print(
                f"[OurMethodMarginWeightedSqrt] 过滤后无标注池大小: {len(filtered_unlabeled)}"
            )
        else:
            filtered_unlabeled = list(unlabeled_indices)
            print(f"[OurMethodMarginWeightedSqrt] 无标注池大小: {len(filtered_unlabeled)}")
        ...
        if filtered_unlabeled:
            ...
            print(
                f"[OurMethodMarginWeightedSqrt] 第一轮选择的标注点数量: {len(selected_indices)}"
            )
            print(
                f"[OurMethodMarginWeightedSqrt] 第一轮覆盖的点数量: {len(covered_indices)}"
            )
        else:
            print("[OurMethodMarginWeightedSqrt] 无标注池为空，无需选择样本")
    ...
    def _subsequent_round_selection(
        ...
        训练用的原始复制次数，并进一步应用 `ceil(sqrt(w))` 压缩。
        """
        if self._should_use_cuda_sampling(device):
            print(f"\n[OurMethodMarginWeightedSqrt] 第{self.round_counter + 1}轮采样")
            (
                selected_indices,
        ...
        print(f"[OurMethodMarginWeightedSqrt] 当前轮次覆盖率: {coverage_ratio:.6f}")
        ...
        if self.enable_early_radius_normalization:
            ...
            if normalization_applied:
                ...
                self.last_coverage = coverage_ratio
                print(
                    "[OurMethodMarginWeightedSqrt] 早期半径归一化已触发: "
                ...
                print(
                    "[OurMethodMarginWeightedSqrt] 早期半径归一化未触发: "
        ...
        print(f"[OurMethodMarginWeightedSqrt] 过滤后无标注池大小: {len(filtered_unlabeled)}")
        ...
        print(
            "[OurMethodMarginWeightedSqrt] 预估半径统计: "
        ...
        selected_local_positions = self._greedy_select_candidate_positions_from_cover_data(
            ...
            query_budget=self.query_budget,
            progress_desc=f"[OurMethodMarginWeightedSqrt] 第{self.round_counter + 1}轮选择进度",
            log_prefix="OurMethodMarginWeightedSqrt"
        )
        selected_indices = [
